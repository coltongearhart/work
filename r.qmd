...
# -> MTD claims data
data_claims_mtd_raw <- DBI::dbGetQuery(con, "select * from XXX.CLT.tiering_analysis_BPCLM_raw") %>% 
  janitor::clean_names() %>% 
  mutate(data_pull_date = ymd(data_pull_date),
         peril_code = trimws(peril_code))

# -> transactional claims data
data_claims_tr_raw <- DBI::dbGetQuery(con, "select * from XXX.CLT.tiering_analysis_BPCLMTR_raw") %>% 
  janitor::clean_names() %>% 
...
data_policy <- DBI::dbGetQuery(con, "select * from XXX.CLT.tiering_analysis_BPPOL") %>% 
  janitor::clean_names() %>% 
  mutate(data_pull_date = data_pull_date %>% as.character.POSIXt %>% as.Date %>% ymd,
...
![](files/large-risks-analysis.png)
...
![](files/policy-life-analysis.png)
...
### Random loss analysis

![](files/random-loss-analysis.png)
...
![](files/restaurant-audit-analysis.png)
...
data_audits_raw <- readxl::read_excel("Audit Review Spreadsheet.xlsx", sheet = 1, skip = 2) %>% 
  janitor::clean_names() %>% 
...
![](files/indication.png)

![](files/indication-2.png)
