...
# setup an in-memory empty database within R
# NOTE: need this just to get the book to render with sql code chunks
library(DBI)
...
from openquery(XXX,'select current date as data_pull_date, o.PLCNUM as Pol_Num, o.OCCMNR as Claim_Num, o.COMP# as Company, o.OCPLST as Pol_State, o.TMEFDT as Term_Eff_Date, o.ACCDAT as Acc_Date, o.REPDAT as Rep_Date, o.PERILCD as Peril_Code,
							...
							from XXX.XXX.XXX o -- claim occurrence
								left join XXX.XXX.XXX o2 -- claim financials
...
select *
into #temp
from openquery(XXX, 'select pt.POLICY_NUMBER, pt.COMPANY, pt.STATE, case when left(c.CLAIM_NO,2)=''BP'' then right(c.CLAIM_NO,10) else c.CLAIM_NO end as CLAIM_NO, c.REPORTED_DATE, c.LOSS_DATE, ce.ISO_CODE, cc.COVERAGE_CODE, 
						   ...
						   from		(select CLAIM_ID, CLAIM_POLICY_TERM_ID, CLAIM_NO, REPORTED_DATE, LOSS_DATE, PCS_CAT_EVENT_ID, AO_CAT_EVENT_ID
								...
								left join	(select sum(case when ACTIVITY_CODE=2001001 then ALLOCATED else 0 end) as Case_Res,
											...
											from XXX.XXX.XXX where deleted_yn=''N'' and TRANS_STATUS=''PROCESSED'' 
...
select * 
into #BPPOL
from openquery(XXX,'select a.SMREFN as SRS_Ref, a.SMPRFX as P_Prefix, a.SMPLNR as P_Num, u.UMUNIT as Unit_Num, c.PMSTAT as P_Status, a.SMTRCD as Trans_Code, c.PMPLST as Pol_State, c.PMOEFFDTE as Orig_Eff_Date, cast(digits(s.PSEFCN) || right(digits(s.PSEFYY),2) || ''-'' || right(digits(s.PSEFMM),2) || ''-'' || right(digits(s.PSEFDD),2) as date) as Term_Eff_Date,
							...
							from XXX.XXX.XXX a --stat master file, ledger
								left join XXX.XXX.XXX s --policy specific activity events, ledger
									on a.SMREFN=s.PSREFN
								left join XXX.XXX.XXX c --policy master file, latest info
									on a.SMPRFX=c.PMPRFX and a.SMPLNR=c.PMPLNR
								left join XXX.XXX.XXX u --general unit level
									on a.SMPRFX=u.UMPRFX and a.SMPLNR=u.UMPLNR
								left join XXX.XXX.XXX i --unit level, commercial 
									on s.PSREFN=i.CPREFN and u.UMUNIT=i.CPUNIT
								left join XXX.XXX.XXX r --location 
									on s.PSREFN=r.RMREFN and u.UMUNIT=r.RMUNIT 
								left join XXX.XXX.XXX z --class codes 
									on i.SWCZCLFTNKEY=z.CNID and c.PMPLST=z.CLFTN_POLICY_STATE and r.RMRBCD=z.CLFTN_RATE_BOOK_CODE
								left join XXX.XXX.XXX e --entity code 
									on a.SMPRFX=e.BIPRFX and a.SMPLNR=e.BIPLNR
								left join XXX.XXX.XXX d --entity descr 
...
into #credit_score
from openquery(XXX,'select FIPRF as P_Prefix, FIPLNM as P_Num, FIBCS as Credit_Score 
							from XXX.XXX.XXX --exact credit score
...
select * 
into #BPCOV
from openquery(XXX,  'select	a.SMREFN as SRS_Ref, a.SMPRFX as P_Prefix, a.SMPLNR as P_Num, u.UMUNIT as Unit_Num, c.CVCOVG as Cov_Code, c.CVVALU as Cov_Limit, 
									c.CVEXPO as C_Exposure, c.CVDEDU as Deductible, l.CILM1N as Occ_Limit, l.CILM2N as Agg_Limit, l.CILM3N as PCO_Limit
							from XXX.XXX.XXX a --stat master file, ledger
								left join XXX.XXX.XXX u --general unit level
									on a.SMPRFX=u.UMPRFX and a.SMPLNR=u.UMPLNR
								left join XXX.XXX.XXX c --all coverages
									on a.SMREFN=c.CVREFN and u.UMUNIT=c.CVUNIT
								left join XXX.XXX.XXX l
...
select * 
into #BPFEE
from openquery(XXX,  'select a.SMREFN as SRS_Ref, a.SMPRFX as P_Prefix, a.SMPLNR as P_Num, f.ADTEFFDTE as Term_Eff_Date, case when sum(f.ADAMTTOT) is null then 0 else sum(f.ADAMTTOT) end as IF_Fees
						from XXX.XXX.XXX a --stat master file, ledger
								left join XXX.XXX.XXX s --policy specific activity events, ledger
									on a.SMREFN=s.PSREFN
								left join XXX.XXX.XXX u --general unit level
									on a.SMPRFX=u.UMPRFX and a.SMPLNR=u.UMPLNR
							left join XXX.XXX.XXX f --policy level fee table
